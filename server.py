...
IT桔子投融资 API (47:8000/api/itjuzi):
  - GET /events                        事件列表（行业/轮次/日期筛选）
  - GET /events/{id}                   事件详情
  - GET /stats                         统计聚合
...
ITJUZI_BASE = "http://47.99.57.152:8000/api/itjuzi"
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "投融资数据源：IT桔子（先进制造/新材料等行业的投融资事件、公司详情、统计聚合）。"
        "投融资支持按赛道查询（新材料=先进制造+子行业精确匹配）、按行业筛选、关键词搜索、事件详情、统计概览。"
    ),
    host="0.0.0.0",
    port=58178,
)
...
def _time_window_to_days(time_window: str) -> int:
    """将资讯 time_window 格式转换为天数。"""
    mapping = {"1h": 1, "6h": 1, "24h": 1, "7d": 7, "30d": 30}
    return mapping.get(time_window, 14)


@mcp.tool()
def d8q_get_financing(
    track: str = "",
    industry: str = "",
    keyword: str = "",
    round_name: str = "",
    ...
    """获取投融资事件列表。支持按赛道/行业/关键词/轮次筛选，兼容资讯赛道参数。

    三种查询模式（优先级从高到低）：
    1. track=赛道名（如"新材料"）：自动映射到 itjuzi 行业，新材料赛道还会精确匹配子行业
    2. industry=行业名：直接筛选 itjuzi 一级行业（先进制造/人工智能/医疗健康等）
    3. keyword=关键词：按公司名/简介/子行业模糊搜索

    参数：
    - track: 赛道名称，如"新材料"、"人工智能"、"先进制造"（与资讯赛道兼容）
    - industry: 一级行业名，如"先进制造"、"人工智能"、"医疗健康"（与 track 二选一）
    - keyword: 搜索关键词（在公司名/简介/子行业中匹配）
    - round_name: 轮次筛选，如"A轮"、"天使轮"、"战略投资"
    - time_window: 时间窗口，"1h"/"6h"/"24h"/"7d"/"30d"（默认30d，与资讯参数一致）
    - limit: 返回条数（默认20，最大50）

    返回每条事件的：公司名、行业、子行业、轮次、金额、投资方、日期、匹配方式。
    """
    limit = min(limit, 50)
    days = _time_window_to_days(time_window)

    end_date = date_type.today().isoformat()
    start_date = (date_type.today() - timedelta(days=days)).isoformat()

    # 模式 1: track=新材料 → 用 material_integration 精确匹配
    if track and track in ("新材料", "碳纤维", "合成生物"):
        data = _get(f"{DATAAGENT_BASE}/api/itjuzi/material", params={
            "days": days,
            "page_size": limit,
        })
        if data is None:
            # material 端点不存在时 fallback 到 industry 查询
            mapped = _TRACK_INDUSTRY_MAP.get(track, track)
            data = _get(ITJUZI_BASE + "/events", params={
                "industry": mapped,
        ...
        if data is None:
            return "错误：无法连接投融资数据服务"
    else:
        # 模式 2/3: industry 或 keyword
        params = {
            "start_date": start_date,
            "end_date": end_date,
            "page_size": limit,
        ...
        elif track and track in _TRACK_INDUSTRY_MAP:
            params["industry"] = _TRACK_INDUSTRY_MAP[track]
        if round_name:
            params["round"] = round_name
        data = _get(ITJUZI_BASE + "/events", params=params)
    ...
    if not items:
        desc = track or industry or keyword or "全部"
        return f"「{desc}」暂无投融资事件"

    lines = [f"共 {total} 条投融资事件：\n"]
    for i, ev in enumerate(items[:limit], 1):
        company = ev.get("company_name", "-")
        ind = ev.get("industry", "-")
        sub = ev.get("sub_industries") or []
        sub_str = ", ".join(sub) if isinstance(sub, list) else str(sub or "")
        rnd = ev.get("round", "-")
        amt = ev.get("amount_text") or "-"
        invs = ev.get("investors") or []
        inv_names = ", ".join(x.get("name", "") for x in invs[:3]) if isinstance(invs, list) else str(invs or "")
        if isinstance(invs, list) and len(invs) > 3:
            inv_names += "..."
        date_str = str(ev.get("event_date", ""))[:10]
        match_type = ev.get("match_type", "")
        match_tag = f" [{match_type}]" if match_type else ""
        sub_part = f" > {sub_str}" if sub_str else ""
        lines.append(
            f"{i}. **{company}**{match_tag}\n"
            f"   {ind}{sub_part} | {rnd} {amt} | {date_str}\n"
            f"   投资方: {inv_names}"
        )
...
@mcp.tool()
def d8q_get_financing_detail(event_id: int) -> str:
    """获取投融资事件详情：公司简介、子行业、完整投资方、金额估值等。

    参数：
    - event_id: 事件 ID（从 d8q_get_financing 结果中获取）

    返回完整事件信息、投资方列表、关联公司详情。
    """
    ev = _get(f"{ITJUZI_BASE}/events/{event_id}")
    if not ev:
        return f"错误：无法获取投融资事件 {event_id}"
    ...
    rnd = ev.get("round", "-")
    amt = ev.get("amount_text") or "-"
    amt_cny = ev.get("amount_cny_est")
    amt_cny_str = f"{amt_cny:,.0f} CNY" if amt_cny else "-"
    date_str = str(ev.get("event_date", ""))[:10]
    brief = ev.get("company_brief") or ""
    source_url = ev.get("source_url") or ""
    valuation = ev.get("valuation") or "-"
    currency = ev.get("currency") or "-"

    lines = [f"## {company} — {rnd} {amt}\n"]
    lines.append(f"- 行业: {ind}" + (f" > {sub_str}" if sub_str else ""))
    lines.append(f"- 日期: {date_str}")
    lines.append(f"- 金额: {amt} ({amt_cny_str})")
    if valuation != "-":
        lines.append(f"- 估值: {valuation}")
    if currency != "-":
    ...
    if isinstance(invs, list) and invs:
        ...
        for inv in invs:
            name = inv.get("name", "")
            is_lead = " (领投)" if inv.get("is_lead") else ""
            lines.append(f"  - {name}{is_lead}")

    return "\n".join(lines)


@mcp.tool()
def d8q_get_financing_stats(
    industry: str = "",
    time_window: str = "30d",
) -> str:
    """获取投融资统计概览：事件数、总金额、行业分布、轮次分布。

    参数：
    - industry: 行业筛选（如"先进制造"），留空返回全行业统计
    - time_window: 时间窗口，"7d"/"30d"（默认30d，与资讯参数一致）

    返回事件总数、总金额、行业/轮次/月度分布。
    """
    days = _time_window_to_days(time_window)
    end_date = date_type.today().isoformat()
    start_date = (date_type.today() - timedelta(days=days)).isoformat()
    ...
    data = _get(f"{ITJUZI_BASE}/stats", params=params)
    if not data:
        return "错误：无法获取投融资统计数据"

    total = data.get("total_events", 0)
    total_amt = data.get("total_amount_cny", 0)
    period = f"{data.get('start_date', '')} ~ {data.get('end_date', '')}"

    lines = [f"## 投融资统计概览\n"]
    lines.append(f"- 统计周期: {period}")
    lines.append(f"- 事件总数: {total}")
    lines.append(f"- 总金额: {total_amt:,.0f} CNY")

    by_industry = data.get("by_industry", [])
    if by_industry:
        lines.append("\n**行业分布 Top 10**:")
        for d in by_industry:
            lines.append(f"  - {d.get('name', '-')}: {d.get('count', 0)} 条")
    ...
    if by_month:
        lines.append("\n**月度趋势**:")
        for d in by_month:
            lines.append(f"  - {d.get('month', '-')}: {d.get('count', 0)} 条")
